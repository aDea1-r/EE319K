...
// Grading engine for Lab2, Spring 2020
...
#define COUNT2 13
const int16_t Input2[COUNT2] = {-273,-202,-131,-60,11,82,153,224,295,366,437,508,538};
const int16_t Correct2[COUNT2] = {-459,-332,-204,-76,52,180,307,435,563,691,819,946,1000};
...
TestCase3Type Test3[COUNT3]={
  { {38,37,35,34,32,32,32,29,27,26,24,24,24,23,22,22,18,16,12,12,-7},21,True},
  { {36,37,35,34,32,32,32,29,27,26,24,23,22,22,18,16,12,12,-7},      19,False},  
  { {81,81,80,80,80,80,80,80,80,80},                                 10,True},
  { {38,32,32,27,24,21,16,13,10, 9, 9,10,28,29,35,29,27,16, 7, 4, 2},21,False},
  { {100},                                                            1,True}  
};
...
int main(void){ 
  ...
  UART_OutString("Test of your CtoF...");
  for(n=0; n<COUNT2; n++){
    result = CtoF(Input2[n]);
    if((result-Correct2[n]<-1)||(result-Correct2[n]>1)){
      if(fails2<3){
        UART_OutString("\n\r No, Input = ");
        UART_OutSDec(Input2[n]);
        UART_OutString("C, Correct Output= ");
        UART_OutSDec(Correct2[n]);  
        UART_OutString("F, Your Output= "); 
        UART_OutSDec(result);
        UART_OutString("F");
      }
