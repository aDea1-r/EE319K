...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
    ...
    PUSH{R4-R11, LR}
readOne
	LDR R3, = SSI0_SR_R  ;Step 1
	LDR R2, [R3]	  ;Get data from address
	MOV	R1, R2
	AND	R1, #0x10     ;Bitmask 0x00010000 Isolate bit 4
	CMP R1, #0X10	  ;Step 2 loop if bit 4 high
	BEQ readOne
	LDR R1, = GPIO_PORTA_DATA_R  ;clear Port A pin 6   Step 3
	LDR R2, [R1]				 ;Get data from address
	AND R2, #0xBF				 ;Bitmask 0x10111111
	STR R2, [R1]                 ;Store updated pin 6 back
	LDR R1, = SSI0_DR_R		 	 ;Step 4 Writing command to SSIO_DR_R
	STR R0, [R1]
readTwo
	LDR R3, = SSI0_SR_R	;Step 5
	LDR R2, [R3]		;Get data from address
	MOV R1, R2
	AND R1, #0x10	   ;Bitmask 0x00010000 Isolate bit 4
	CMP R1, #0x10	   ;Step 6 check if high and wait until low
	BEQ readTwo
    POP{R4-R11, LR}
    BX  LR                          ;   return
...
writeLp
	...
	LDR R2, [R3]	  ;Get data from address
	MOV	R1, R2
	AND	R1, #0x02     ;Bitmask 0x00000010 Isolate bit 1
	CMP R1, #0X00	  ;Step 2 loop if bit 1 low
	BEQ writeLp
